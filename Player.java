public class Player{
      ...
      public Move chooseMove(gameOfGraph G) { 
        int max = -10000;
      ...
      private int eval(gameOfGraph G){

          int i = 0;

          for(int j = 0; j < G.sizeOfGraph(); j++){

            if(G.breakerList.contains(j) && !(G.makerList.contains(j)))
      ...
      int minMax(gameOfGraph G, int depth, int alpha, int beta ) {  
        if(depth == D || G.isFull())  
        	// will upload eval func
